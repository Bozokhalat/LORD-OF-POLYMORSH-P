...
class Oyuncu():
    ...
    def karakter_ekle(self,karakterkodu,kordinatno):
        ...
        if karakterkodu=="1" and  (kordinatno in self.etkin_alan_numaraları) and self.hazine>=30:
            self.hazine-=30
            hak+=1
        ...
        if karakterkodu=="2" and  (kordinatno in self.etkin_alan_numaraları)and self.hazine>=10:
            hak+=1
            self.hazine-=10
            self.muhafızsay+=1
        ...
        if karakterkodu=="3" and  (kordinatno in self.etkin_alan_numaraları)and self.hazine>=20:
            self.hazine-=20
            hak+=1
        ...
        if karakterkodu=="4" and  (kordinatno in self.etkin_alan_numaraları)and self.hazine>=10:
            self.hazine-=10
            hak+=1
        ...
        if karakterkodu=="5" and  (kordinatno in self.etkin_alan_numaraları)and self.hazine>=50:
            self.hazine-=50
            hak+=1
...
def karerenkdegistir(ekran, rengi, sira,isim,sahip="Unkown",can=0,kaynak=0): # numara*renk*kareyazı*sahip*can*kaynak formatı
...
while True:
    ...
    boyut = input("Boyutu giriniz (AxB şeklinde):  En fazla 32x32 en düşük 8x8 ").split("x") # Kullanıcıdan boyut bilgisi al
    rows = int(boyut[0])

    cols = int(boyut[1])
    if (rows >=8 and cols>=8) and (rows <=32 and cols<=32):

        break

    else:

        print("geçerli boyut aralığı girin")
...
while True:
    ...
    if  not mevcutOyuncu=="saldiri":
        ...
        textkaynak=font.render("Kaynak  {}".format(mevcutOyuncu.hazine),True,renkler[mevcutOyuncu.renk])
    ...
    screen.blit(textkaynak,(height+30, 80))
    if mevcutOyuncu=="saldiri":
        ...
        for i in range(1,cols*rows+1):
            renk=satirokuyucu("game.txt",i).split("*")
            if renk[1]=="green":
                green+=1
            elif renk[1]=="red":
                red+=1
        ...
        print("ALAN DURUMU \n "  ,"mavi :{} , kırmızı : {} , sarı : {} , yesil : {}".format(blue,red,yellow,green))    

    
        if red>=cols*rows:
            print("oyunu kırmızı kazandı")
            break
        elif blue>=cols*rows:
            print("oyunu mavi kazandı")
            break
        elif green>= cols*rows:
            print("oyunu yesil kazandı")
            break
        elif yellow>= cols*rows:
            print("oyunu sarı kazandı")
            break
        birliksayisi=0
